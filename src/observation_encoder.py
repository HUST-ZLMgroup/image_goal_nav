...
from src import residual_networks as compact_residual
from src.tensor_transforms import CenterResizeCrop
...
class DualViewEncoder(nn.Module):
    def __init__(
        ...
        if self.obs_transform is not None and not backbone == "clip_feat":
            ...
            observation_space = self.obs_transform.transform_observation_space(
                observation_space, trans_keys=OBSERVATION_MODALITIES
            )
        self._n_input = {k: 0 for k in OBSERVATION_MODALITIES}
        ...
        visual_uuid = OBSERVATION_MODALITIES
        if ((visual_obs_inputs is not None) and (len(visual_obs_inputs) > 0) and (visual_obs_inputs[0] != '*')):
            if isinstance(visual_obs_inputs, list):
                ...
                visual_uuid = visual_obs_inputs.split(MODALITY_SEPARATOR)

        self._id2rgb = {k: False for k in OBSERVATION_MODALITIES}
                ...
                if tied_params is not None:
                    ...
                    if 'simple_cnn' in backbone:
                        ...
                        if 'fast_resnet' in backbone:
                            ...
                            backbone_enc = getattr(compact_residual, restnet_type)
    ...
    def _apply_rgb_jitter(self, rgb_obs, seeds=None):
        rgb_3 = None
        if rgb_obs.size(1) == 3:
            rgb_3 = rgb_obs
                ...
                if self._n_input[v_uuid] > 0:
                    ...
                    if v_uuid in ["rgb", "imagegoal_sensor_v2", "panoramic_rgb_sensor", "instance_imagegoal"] and self._rgb_aug is not None:
                        if v_uuid == "panoramic_rgb_sensor":
                            ...
                            v_observations = self._apply_rgb_jitter(v_observations, seeds)
