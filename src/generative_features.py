"""Feature extraction helpers built on diffusion models."""
...
class FeatureUNet(UNet2DConditionModel):
    def forward(
        self,
...
class SingleStepFeaturePipeline(StableDiffusionPipeline):
    @torch.no_grad()
    def __call__(
        self,
        img_tensor,
...
class PromptedDiffusionFeatureExtractor(DiffusionFeatureExtractor):
