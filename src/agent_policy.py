"""Goal-conditioned actor-critic policy definitions."""
...
from src.tensor_transforms import (
    CenterResizeCrop,
    RandomResizeCrop,
)
from src.observation_encoder import DualViewEncoder

try:
    from superglue.models.matching import Matching
except ImportError:  # Optional path, disabled by the default configuration.
    Matching = None
...
class GoalConditionedPolicy(NetPolicy):
    def __init__(
        self,
        observation_space,
        action_space,
        ...
        obs_transform=CenterResizeCrop(size=(256, 256)),  # noqa : B008
    ...
    # def from_config(cls, config, envs):
        ...
        if ppo_config.random_crop:
            obs_transform = RandomResizeCrop(
                size=(ppo_config.input_size, ppo_config.input_size))
        else:
            obs_transform = CenterResizeCrop(
                size=(ppo_config.input_size, ppo_config.input_size))
        ...
        return cls(
            ...
            context_checkpoint_dir=ppo_config.context_checkpoint_dir
        )


class RecurrentGoalNetwork(Net):
    ...
    def __init__(
        ...
        **kwargs
        ...
        EncoderFactory = DualViewEncoder

        logger.info('Type of observation encoder: {}'.format(EncoderFactory))
        tied_param = {}

        # construct goal encoder
        
        logger.info(f'Create goal encoder : {goal_backbone}')
        if CachedGoalFeatureSensor.cls_uuid in observation_space.spaces:
            self._goal_sensor_uuid = CachedGoalFeatureSensor.cls_uuid
        elif TargetViewSensor.cls_uuid in observation_space.spaces:
            self._goal_sensor_uuid = TargetViewSensor.cls_uuid
        elif ReferenceObjectSensor.cls_uuid in observation_space.spaces:
            self._goal_sensor_uuid = ReferenceObjectSensor.cls_uuid
        elif InstanceImageGoalSensor.cls_uuid in observation_space.spaces:
            self._goal_sensor_uuid = InstanceImageGoalSensor.cls_uuid
        else :
            raise ValueError("Wrong goal sensor")

        if enable_feature_matching:
            self.matching_module = SparseCorrespondenceModule(
                visual_obs_inputs=visual_obs_inputs,
        ...
        if goal_backbone != "none":
            self.goal_visual_encoder = EncoderFactory(
        ...
        self.visual_encoder = EncoderFactory(
...
class SparseCorrespondenceModule(nn.Module):
    def __init__(
            self, 
            visual_obs_inputs, 
            goal_sensor, 
        ...
        if Matching is None:
            raise ImportError(
                "Sparse correspondence features require the optional superglue package."
            )
