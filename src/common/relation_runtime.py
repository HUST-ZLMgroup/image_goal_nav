"""Runtime helpers for training the auxiliary relation model."""
...
def prepare_relation_run(arg_mode='train'):
    args = parse_relation_arguments(arg_mode=arg_mode)
    pretty_print(vars(args))

    torch.set_printoptions(linewidth=100)
    args.num_gpu = configure_visible_devices(args)
...
def restore_relation_model(model, model_path):
    model_dict = model.state_dict()
    pretrained_dict = torch.load(model_path,map_location=torch.device('cuda', torch.distributed.get_rank()))['params']

    if pretrained_dict.keys() == model_dict.keys():  # load from a parallel meta-trained model and all keys match
        print('all state_dict keys match, loading model from :', model_path)
...
def seed_relation_training(seed):
    if seed == 0:
        print(' random seed')
...
def parse_relation_arguments(arg_mode):
