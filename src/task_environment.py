"""Reinforcement-learning environment adapters."""
...
class GoalNavigationRLEnv(habitat.RLEnv):
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        habitat.logger.info('GoalNavigationRLEnv: '
    ...
    def reset(self):
        ...
        self._previous_measure = self._read_shaping_measure()
        return observations

    def _read_shaping_measure(self):
        current_measure = 0.0
    ...
    def get_reward(self, observations):
        ...
        current_measure = self._read_shaping_measure()

        reward += self._previous_measure - current_measure
        self._previous_measure = current_measure

        if self._success_score():
            reward += self._success_score() * self._config.task.success_reward

        return reward

    def _success_score(self):
        return self._env.get_metrics()[self._success_measure_name]

    def get_done(self, observations):
        done = False
        if self._env.episode_over or self._success_score():
            done = True
        return done
...
@habitat.registry.register_env(name="GoalNavigationEnv")
class GoalNavigationGymEnv(gym.Wrapper):
    """
    ...
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        base_env = GoalNavigationRLEnv(config=config, dataset=dataset)
