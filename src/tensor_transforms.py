...
"""Tensor-based observation transforms and action distributions."""
        ...
        if size:
            for key in observation_space.spaces:
                if (
                    ...
                    observation_space.spaces[key] = reshape_box_space(
...
class CenterResizeCrop(nn.Module):
...
def crop_center(img, size, channels_last: bool = False):
...
def reshape_box_space(box: Box, shape) -> Box:
    if box.shape == shape:
        return box
...
class GaussianActionHead(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.logstd = LearnableBias(torch.zeros(num_outputs))
    ...
    def forward(self, x):
        ...
        return DiagonalNormal(action_mean, action_logstd.exp())


class MaskedCategoricalHead(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        super().__init__()
